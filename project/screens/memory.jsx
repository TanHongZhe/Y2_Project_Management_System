// Project Memory screen — TOC + rendered sections with edit affordance.
const { useState: useStateMem, useRef: useRefMem } = React;

function Memory({ data }) {
  const [activeId, setActiveId] = useStateMem(data.memorySections[0].id);
  const [editingId, setEditingId] = useStateMem(null);
  const refs = useRefMem({});
  const scrollRef = useRefMem(null);
        ...
        <div className="memory-doc" ref={scrollRef}>
          ...
          {data.memorySections.map(s => (
            ...
            >
              ...
              <div className="content" contentEditable={editingId === s.id} suppressContentEditableWarning>
...
function renderBlocks(blocks) {
  return blocks.map((b, i) => {
    ...
    if (b.type === "ul") return <ul key={i}>{b.items.map((x, j) => <li key={j}>{renderInline(x)}</li>)}</ul>;
    if (b.type === "ol") return <ol key={i}>{b.items.map((x, j) => <li key={j}>{renderInline(x)}</li>)}</ol>;
    if (b.type === "kv") {
      return (
