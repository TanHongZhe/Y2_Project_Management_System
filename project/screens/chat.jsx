// Chat screen — streaming, citations, boost, tool calls, side rail.
const { useState: useStateChat, useEffect: useEffectChat, useRef: useRefChat, useMemo: useMemoChat } = React;

function Chat({ data, tweaks, setRoute }) {
  const [boost, setBoost] = useStateChat(false);
  const [showDebug, setShowDebug] = useStateChat(false);
  const [railTab, setRailTab] = useStateChat("memory");
  const [draft, setDraft] = useStateChat("");
  const [streaming, setStreaming] = useStateChat(false);
  const [streamText, setStreamText] = useStateChat("");
  const taRef = useRefChat(null);
  const scrollRef = useRefChat(null);

  // Seeded conversation
  const seedMessages = useMemoChat(() => ([
    {
