...
import { AppData, Component } from '@/lib/data';
...
type SortKey = keyof Component;

export default function Components({ data }: ComponentsProps) {
  ...
  function Th({ label, k, align }: { label: string; k: SortKey; align?: string }) {
    return (
      <th onClick={() => toggleSort(k)} style={align ? { textAlign: align as React.CSSProperties["textAlign"] } : undefined}>
        {label}
