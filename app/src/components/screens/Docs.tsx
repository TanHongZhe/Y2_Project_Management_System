...
const DOC_CONTENTS: Record<string, { type: string; body: React.ReactNode }> = {
  "README.md": {
    type: "md",
    body: (
      <>
...
export default function Docs({ data }: DocsProps) {
  ...
  const totalFiles = data.docs.reduce((s, g) => s + g.items.length, 0);
    ...
    <>
      <header className="screen-header">
        <div className="title-block">
          ...
          <h1>Docs <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-faint)", marginLeft: 8 }}>{totalFiles} files · sync&apos;d from /project_docs</span></h1>
        </div>
